...
def format_context_with_distances(
    documents: List[str],
    metadatas: List[Dict],
    distances: List[float],
) -> str:
    """Format retrieved documents into context with retrieval distances."""
...
def main():
    ...
    with st.sidebar:
        ...
        base_url = st.text_input(
            "API Base URL",
            value=os.getenv("OPENAI_BASE_URL", os.getenv("OPENAI_COMPATIBLE_BASE_URL", "")),
            help="Leave blank for OpenAI, or use a compatible provider such as DashScope, Kimi, Zhipu, DeepSeek, or Doubao"
        )
        openai_key = st.text_input(
            ...
            help="Enter the key for your selected OpenAI-compatible provider"
        )
        ...
        if not openai_key:
            ...
            if base_url:
                ...
                os.environ["CHROMA_OPENAI_BASE_URL"] = base_url
            os.environ["CHROMA_OPENAI_API_KEY"] = openai_key
        
        # Model selection
        model_choice = st.selectbox(
            "Chat Model",
            options=["qwen-plus", "moonshot-v1-32k", "glm-4-flash", "deepseek-chat", "doubao-seed-1-6", "gpt-3.5-turbo", "gpt-4"],
            help="Choose the chat model for responses"
        )
        ...
        mission_filter = st.selectbox(
            ...
            help="Restrict retrieval to one mission or search all missions",
        )
        ...
        with st.chat_message("assistant"):
            with st.spinner("Searching documents and generating response..."):
                ...
                if docs_result and docs_result.get("documents"):
                    context = format_context_with_distances(
                        docs_result["documents"][0],
                        docs_result["metadatas"][0],
                        docs_result.get("distances", [[]])[0],
                    )
