...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def __init__(self, 
        ...
        self.openai_base_url = openai_base_url or os.getenv("OPENAI_BASE_URL") or os.getenv("OPENAI_COMPATIBLE_BASE_URL")
        self.client = OpenAI(api_key=openai_api_key, base_url=self.openai_base_url)
        ...
        try:
            self.embedding_function = OpenAIEmbeddingFunction(**embedding_kwargs)
        except TypeError:
        ...
        self.collection = self.chroma_client.get_or_create_collection(
            ...
            metadata={"description": "NASA mission transcripts and documents"},
        )
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        while start < len(text):
            ...
            if target_end < len(text):
                ...
                if sentence_break > start + int(self.chunk_size * 0.5):
    ...
    def generate_document_id(self, file_path: Path, metadata: Dict[str, Any]) -> str:
        ...
        safe_source = "".join(
            char if char.isalnum() else "_" for char in source.lower()
        ).strip("_")
        return f"{mission}_{safe_source}_{path_hash}_chunk_{chunk_index:04d}"
    ...
    def scan_text_files_only(self, base_path: str) -> List[Path]:
        ...
        # Define directories to scan. The starter repo ships these under
        # data_text/, while the classroom text refers to data/.
        data_dirs = [
            'apollo11',
            'apollo13',
            'challenger'
        ...
        search_roots = [base_path, base_path / 'data', base_path / 'data_text']
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        if update_mode == "replace":
            ...
            if existing_file_ids:
                ...
                logger.info(
                    "Deleted %s existing chunks for %s before replacement",
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        for file_path in files_to_process:
            try:
                ...
                logger.info(
                    "Processed %s: %s chunks, added=%s, updated=%s, skipped=%s",
                    file_path,
                    len(documents),
                    add_stats["added"],
                    add_stats["updated"],
                ...
                stats["errors"] += 1
                logger.exception(f"Failed processing file: {file_path}")
...
def main():
    ...
    parser.add_argument(
        '--openai-key',
        default=os.getenv("OPENAI_API_KEY") or os.getenv("OPENAI_COMPATIBLE_API_KEY"),
        help='OpenAI API key. Defaults to OPENAI_API_KEY or OPENAI_COMPATIBLE_API_KEY',
    )
    parser.add_argument('--openai-base-url', help='OpenAI-compatible API base URL')
    ...
    if not args.openai_key:
        parser.error("--openai-key is required unless OPENAI_API_KEY is set")
